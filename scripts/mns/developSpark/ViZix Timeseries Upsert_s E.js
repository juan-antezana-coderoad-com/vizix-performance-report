...
function (options) {
    ...
    // var end = new Date(Math.floor(now.getTime() / 60000) * 60000 - 4 * 3600 * 1000);
    var end = now;
            ...
            {"$project" : {
                ...
                "source" : 
                {
                        "$cond": [
                            {
                                "$and": [
                                    {"$ne": ["$value.source.value", null]},
                                    {"$ne": ["$value.source.value", undefined]}
                                ]
                            }, "$value.source.value", "Source Not Found"]
                },
                //"time" : "$time"
